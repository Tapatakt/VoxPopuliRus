...
UPDATE Language_RU_RU
SET Text = '[COLOR_POSITIVE_TEXT]Крепостное право[ENDCOLOR][NEWLINE][ICON_BULLET]+1 [ICON_CULTURE] культуры и [ICON_PEACE] веры в городах за каждых 4 [ICON_CITIZEN] жителей не-специалистов.[NEWLINE][ICON_BULLET]+2 [ICON_PRODUCTION] производства и +1 [ICON_GOLD] золота от пастбищ.'
WHERE Tag = 'TXT_KEY_POLICY_FREE_RELIGION_HELP';

UPDATE Language_RU_RU
SET Text = 'Крепостное право — это статус многих крестьян при феодализме, в частности относящийся к поместной системе. Крепостничество в основном развилось в Европе в эпоху высокого средневековья и в некоторых странах продолжаломь до середины XIX века. Крепостные, занимавшие участок земли, должны были работать на лорда поместья, владеющего этой землей. Взамен они имели право на защиту, правосудие и право возделывать определенные поля в поместье для поддержания своего собственного пропитания. Крепостные часто должны были не только работать на полях лорда, но и в его шахтах и лесах и поддерживать дороги. Поместье составляло базовую единицу феодального общества, и лорд поместья и вилланы, и в определенной степени крепостные, были связаны юридически: налогообложением в случае первых и экономически и социально в последних.'
...
UPDATE Language_RU_RU
SET Text = '[COLOR_POSITIVE_TEXT]Индустрия[ENDCOLOR] предоставляет бонусы империям, сосредоточенным на [ICON_GOLD] золоте и [ICON_PRODUCTION] производстве.[NEWLINE][NEWLINE][COLOR_POSITIVE_TEXT]Принятие Игдустрии даёт:[ENDCOLOR][NEWLINE][ICON_BULLET]+2 [ICON_INTERNATIONAL_TRADE] торговых пути.[NEWLINE][ICON_BULLET]-5% [ICON_INVEST] золота, необходимого для покупок.[NEWLINE][NEWLINE][COLOR_POSITIVE_TEXT]Каждая принятая политика Индустрии даёт:[ENDCOLOR][NEWLINE][ICON_BULLET]-5% [ICON_INVEST] золота, необходимого для покупок.[NEWLINE][NEWLINE][COLOR_POSITIVE_TEXT]Принятие всех политик Индустрии даёт:[ENDCOLOR][NEWLINE][ICON_BULLET]Открывает возможность строительства [COLOR_POSITIVE_TEXT]Бродвея[ENDCOLOR].[NEWLINE][ICON_BULLET]+3 [ICON_HAPPINESS_1] счастья за каждый уникальный принадлежащий роскошный ресурс.[NEWLINE][ICON_BULLET]+2 [ICON_PRODUCTION] производства и [ICON_GOLD] золота от специалистов.[NEWLINE][ICON_BULLET]Позволяет покупать [ICON_GREAT_MERCHANT] великих торговцев за [ICON_PEACE] веру, начиная с Промышленной эпохи.'
WHERE Tag = 'TXT_KEY_POLICY_BRANCH_COMMERCE_HELP';

UPDATE Language_RU_RU
SET Text = 'Индустрия — это производство товаров или услуг в рамках экономики. В период Промышленной революции в европейских и североамериканских странах Индустрия вышла на передний план, потеснив прежние меркантилистские и феодальные экономики. Это произошло благодаря множеству последовательных быстрых технологических достижений, таких как производство стали и угля. После Промышленной революции около трети мирового экономического производства приходится на обрабатывающие промышленности. Многие развитые страны и многие развивающиеся/полуразвитые страны (Китай, Индия и т.д.) значительно зависят от обрабатывающей промышленности. Индустрии, страны, в которых они находятся, и экономики этих стран связаны сложной паутиной взаимозависимости.'
